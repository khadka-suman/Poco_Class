...
namespace POCOGenerator.DomainServices
{
    public class SqlParser
    {
        ...
        public SqlParser(string connectionString, string sql)
        {
            ...
            //var flag = ParseFlagParameter(sql);
        ...
        private string ParseFlagParameter(string sql)
        {
            var flagValue = string.Empty;
            sql = sql.ToUpper().Trim();
            var posFlag = sql.IndexOf("@FLAG");
            if (posFlag >= 0)
            {
                var posEqual = sql.IndexOf("=", posFlag);
                if (posEqual > posFlag)
                {
                    var endOfLine = sql.IndexOf("\n", posEqual);
                    if (endOfLine > posEqual)
                    {
                        flagValue = sql.Substring(posEqual + 1, endOfLine - posEqual - 1).Trim();
                    }
                    else
                    {
                        flagValue = sql.Substring(posEqual + 1).Trim();
                    }
                }
            }
            return flagValue;
        ...
        public static List<string> GetStoredProcedures(string connectionString)
        {
            using (var con = new SqlConnection(connectionString))
            {
                con.Open();
                var procedures = new List<string>();
                var dt = con.GetSchema("Procedures");
                foreach (DataRow row in dt.Rows)
                {
                    var database = row["ROUTINE_SCHEMA"].ToString();
                    var name = row["ROUTINE_NAME"].ToString();
                    Debug.WriteLine("{0}, {1}", database, name);

                    // Adjust conditions if necessary based on your database schema
                    if (database == "dbo") // Adjust schema condition as per your database setup
                    {
                        procedures.Add(name);
                    }
                }
                return procedures.OrderBy(p => p).ToList();
            }
        }
